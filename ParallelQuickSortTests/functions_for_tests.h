...
T create_rand_and_ParallelQuickSort(T& input)
{
	...
	T res = input;
	ParallelQuickSort::sort(res.begin(), res.end());
	return res;
}
template<typename T>
void test_empty_container() {
	...
	T expected;
	expected = input;
	ParallelQuickSort::sort(result.begin(), result.end());
...
T* create_rand_and_ParallelQuickSort_for_array(T*& input)
{
	...
	ParallelQuickSort::sort(&res[0], &res[NUMBER_OF_ELESMS]);
	return res;
}
template<typename T>
...
void array_test_empty_array() {
	...
	T* result = nullptr;
	T* expected = nullptr;
	ParallelQuickSort::sort(&result, &result);
...
void array_test_reverse_sorted_numbs() {
	...
	memcpy(expected, input, NUMBER_OF_ELESMS * sizeof(T));
	ParallelQuickSort::sort(&result[0], &result[NUMBER_OF_ELESMS]);
	std::sort(&expected[0], &expected[NUMBER_OF_ELESMS]);
	my_and_expected_result_compare_for_array(result, expected, 0);
	delete[] input;
	delete[] result;
