...
//		threads.emplace_back(std::move(thr));
//	}
//	auto func = [first, &part_size, &remainder, comp]() { Qsort(first + (number_of_threads - 1) * part_size, part_size + remainder, comp); };
//		std::thread thr(func);
//		threads.emplace_back(std::move(thr));
//
//	for (auto& thr : threads) {
...
//	size_t number_of_threads = 6/*std::thread::hardware_concurrency()*/;
//	if (number_of_threads == 0) number_of_threads = 2;
//
//	size_t part_size = input_size / number_of_threads;
//	size_t remainder = input_size % number_of_threads;
